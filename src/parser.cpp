...
namespace mahjong::score_calculator::parser {
  namespace regular {
    Tiles from_mpsz_to_tiles(const std::string& mpsz)
    {
      Tiles tiles;
      int base = 0;
    ...
    std::tuple<Hand, Melds> from_mpsz_to_hand_melds(const std::string& mpsz)
    {
      ...
      for (std::sregex_iterator it(std::begin(mpsz), std::end(mpsz), re), end; it != end; ++it) {
        const auto s = it->str();
        const auto tiles = from_mpsz_to_tiles(s);
        // 暗槓
    ...
    {
      static const std::regex re("(?:(?:[1-9]|r5)+[mps]|[1-7]+z)+(?:\\[(?:(?:[1-9]|r5)+[mps]|[1-7]+z)+\\]|\\[\\[(?:(?:[1-9]|r5)+[mps]|[1-7]+z)+\\]\\])*");
